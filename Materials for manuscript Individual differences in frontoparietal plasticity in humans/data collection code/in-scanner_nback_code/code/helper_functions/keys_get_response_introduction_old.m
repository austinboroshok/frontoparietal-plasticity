function [dat,keys] = keys_get_response_introduction_old(w,keys,dat,trial,blockTrial,nback,sound_index,TrialStartTime)
%
%
...
if any([yes no]) && keys.isDown == 0
    ...
    if blockTrial > nback
        ...
        if response == 1 && sound_index == dat.trials.sound_index(trial-nback)
            ...
            DrawFormattedText(w, ['YES (' dat.scr.response_mapping{1} ')'], 'center', dat.scr.y_center_pix - dat.scr.y_center_pix/8, [0 255 0]);
        ...
        elseif response == 2 && ~(sound_index == dat.trials.sound_index(trial-nback))
            ...
            DrawFormattedText(w, ['NO (' dat.scr.response_mapping{2} ')'], 'center', dat.scr.y_center_pix + dat.scr.y_center_pix/8, [0 255 0]);
         ...
         elseif response == 1 && ~(sound_index == dat.trials.sound_index(trial-nback))
            ...
            DrawFormattedText(w, ['YES (' dat.scr.response_mapping{1} ')'], 'center', dat.scr.y_center_pix - dat.scr.y_center_pix/8, [255 0 0]);
        ...
        elseif response == 2 && sound_index == dat.trials.sound_index(trial-nback)
            ...
            DrawFormattedText(w, ['NO (' dat.scr.response_mapping{2} ')'], 'center', dat.scr.y_center_pix + dat.scr.y_center_pix/8, [255 0 0]);
