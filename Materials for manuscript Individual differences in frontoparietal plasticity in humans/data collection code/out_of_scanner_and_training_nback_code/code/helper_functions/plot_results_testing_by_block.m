function [omitted_trials,percent_correct,hits,false_alarms,misses,crs,dp,median_rt] = plot_results_testing_by_block(dat, datapath, sub, block_num)
%
...
if any(isnan(dat.trials.nBackTrue))
    ...
    outputname  = strrep([datapath 'BPD' sub '/CORRECTED_' dat.fileName],'mat','csv');
...
for b = 1:length(dat.nbacks)
    ...
    trial_inds = trial_inds(block_range);

    %calculate proportion of missed trials
    omitted_trial_inds = isnan(dat.trials.resp(block_range));
    ...
    block_correct_trials = dat.trials.isCorrect(block_range);
    
    % percent correct of trials with a valid response
    percent_correct(b)  = sum(block_correct_trials(valid_trial_inds) == 1)/sum(valid_trial_inds);
    
    % get indices of signal and noise trials
    signal_trial_inds = dat.trials.nBackTrue == 1;
    signal_trial_inds = signal_trial_inds(block_range);
    noise_trial_inds = dat.trials.nBackTrue  == 0;
    noise_trial_inds = noise_trial_inds(block_range);
    
    % hits and fa's are calculated for signal and noise trials for this nback, with missed key presses excluded
    hits(b)         = sum(block_correct_trials(valid_trial_inds & signal_trial_inds))/sum(valid_trial_inds & signal_trial_inds);
    false_alarms(b) = sum(~block_correct_trials(valid_trial_inds & noise_trial_inds))/sum(valid_trial_inds & noise_trial_inds);
    
    % misses and correct rejections
    misses(b)   = sum(~block_correct_trials(valid_trial_inds & signal_trial_inds))/sum(valid_trial_inds & signal_trial_inds);
    crs(b)      = sum(block_correct_trials(valid_trial_inds & noise_trial_inds))/sum(valid_trial_inds & noise_trial_inds);
    
    block_respTime = dat.trials.respTime(block_range);
    
    % median of reaction time in seconds
    median_rt(b)          = median(block_respTime(valid_trial_inds));
...
omitted_average = sum(omitted_trials)/3;
